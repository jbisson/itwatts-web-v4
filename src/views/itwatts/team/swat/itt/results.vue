...
import ITTResultsComponent from "@/components/itwatts/itt/ITTResults.vue";
...
const breadcrumbs = ref([
  {
    text: t('ittResults.pageCategory'),
    disabled: false,
    to: "#",
  },
  {
    text: t('ittResults.pageTitle', { serie: route.params.serie }),
    disabled: true,
  },
]);
...
async function refresh() {
  ...
  breadcrumbs.value[1].text = t('ittResults.pageTitle', { serie: route.params.serie });
  
  errorAlert.value = '';

  try {
    loading.value = true;
    const response = await axios.get(`${config.serverApi.publicHostname}/v1/zp-race-serie-report/itt_serie${route.params.serie}`, {
      withCredentials: true
    });
    overallRiderResults.value = response.data.data.overall_riders_result_per_category;
...
watch(() => route.params.serie, (newSerie, oldSerie) => {
  refresh();
  }
)
...
<template>
      ...
      <v-card>
        <v-card-text>      
          <v-radio-group v-model="pickedEvent">
            ...
            <v-radio v-on:change="onRaceChange" v-for="(raceResult, index) in racesResult.value" :label="raceResult.metadata.league_name" :value="index" :key="index"></v-radio>
          </v-radio-group>      
        </v-card-text>
      </v-card>
    </v-col>
  ...
  <v-row>
    <v-col cols="12" sm="12" lg="12">
      <ITTResultsComponent :results="selectedResult.value" :overallResults="overallResults"/>
      </v-col>
  </v-row>
</template>
