...
const page = ref({ title: t('adminSwatMembers2023.pageTitle') });
...
const breadcrumbs = ref([
  {
    text: t('adminSwatMembers2023.pageCategory'),
    disabled: false,
    to: "#",
  },
  {
    text: t('adminSwatMembers2023.pageTitle'),
    disabled: true,
  },
]);
...
const overallHeaders: Header[] = reactive([
  ...
  { text: 'ZP last synced', value: "zp_last_synced", sortable: true },
  { text: 'Zwift 5.W.4.T club status', value: "zwift_status", sortable: true },
  { text: 'Zwift last synced', value: "zwift_last_synced", sortable: true },
  { text: 'ZP primary team', value: "zp_primary_team_name", sortable: true },
  { text: 'Discord roles', value: "discord_roles", sortable: true },
]);
...
async function refresh() {
  ...
  try {    
    const response = await axios.get<ResponseData>(`${config.serverApi.publicHostname}/v1/users?groups=swat_2023_2024&additionalFields=zp_profile&excludedFields=zp_profile(events)&sortBy=first_name,last_name`,
    {
      withCredentials: true
    });
    
    /*const response2025 = await axios.get<ResponseData>(`${config.serverApi.publicHostname}/v1/users?groups=swat_2024_2025&additionalFields=zp_profile&excludedFields=zp_profile(events)&sortBy=first_name,last_name`,
    {
      withCredentials: true
    });*/
    //const swat2025Map = new Set(response2025.data.data.map((a) => a.zp_id));

    usersExport.value = `${exportUser().headers.toString()}\n`;

    for (const user of response.data.data) {
      /*if (swat2025Map.has(user.zp_id)) {
        continue;
      }*/
      ...
      if (user.zwift_profile && user.zwift_profile.clubs && user.zwift_profile.clubs.length > 0) {
...
async function exportMembres() {
  ...
  const universalBOM = "\uFEFF";
  const file = new File([universalBOM + usersExport.value], `${dateNow} membres-5w4t.csv`, {
    type: 'text/csv',
  })
      ...
      <v-card>
        <v-card-text>
          <div class="d-flex align-center mb-4">
            <h2 class="text-h5">{{ $t('adminSwatMembers2023.pageTitle') }}</h2>
          </div>
