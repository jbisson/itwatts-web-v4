...
const page = ref({ title: t('adminSwatGuests2023.pageTitle') });
...
const breadcrumbs = ref([
  {
    text: t('adminSwatGuests2023.pageCategory'),
    disabled: false,
    to: "#",
  },
  {
    text: t('adminSwatGuests2023.pageTitle'),
    disabled: true,
  },
]);
...
async function refresh() {
  ...
  try {    
    ...
    for (const user of response.data.data) {
      ...
      if (user.roles.includes('SWAT_GUEST_MEMBER')) {
      ...
      <v-card>
        <v-card-text>
          <div class="d-flex align-center mb-4">
            <h2 class="text-h5">{{ $t('adminSwatGuests2023.pageTitle') }}</h2>
          </div>
