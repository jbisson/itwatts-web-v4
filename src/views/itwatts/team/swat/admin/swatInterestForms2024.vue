...
import { exportUser, exportInterestSwat2024Form } from '@/utils/export';

const { t, locale } = useI18n({ useScope: 'global' });
const page = ref({ title: t('swatInterestForms2024.pageTitle') });
...
const breadcrumbs = ref([
  {
    text: t('swatInterestForms2024.pageCategory'),
    disabled: false,
    to: "#",
  },
  {
    text: t('swatInterestForms2024.pageTitle'),
    disabled: true,
  },
]);
...
async function refresh() {
  ...
  try {
    ...
    usersExport.value = `${exportInterestSwat2024Form().headers.concat(exportUser().headers).toString()}\n`;
    for (const form of formsResponse.data.data) {
      ...
      const formObj = 
        {
          ...
          wrongValueQuestions: false,
          admissionProcessStatus: swatUsersMap.get(form.user_id).groups.includes('swat_2024_2025') ? '✅' : '❌',
        };
        
        if (formContent.integrityValuesAuthenticPerformances.toString() !== 'yes' ||
          formContent.integrityValuesLowInterestPerformanceValidation.toString() !== '' ||
          formContent.integrityValuesAcceptTests.toString() !== 'yes'
        ) {
          formObj.wrongValueQuestions = true;
        }
      ...
      usersExport.value += `${exportInterestSwat2024Form(formContent).value.concat(exportUser(swatUsersMap.get(form.user_id)).value).toString()}\n`;
    }    
  } catch (err: any) {
          ...
          <div class="align-center">
            <EasyDataTable
                ...
                <template #item-admin_action="{ status, user_id, wrongValueQuestions, onboarding_flow, admissionProcessStatus }">
                  ...
                  <v-btn density="default" v-if="!status" @click="changedStatusForm(user_id, 'recommended')" icon="mdi-check" size="x-small" :disabled="wrongValueQuestions">
                    <v-icon color="grey-lighten-1">mdi-check</v-icon>
                    <v-tooltip activator="parent">Recommander</v-tooltip>
                  </v-btn>
                  <v-btn density="default" v-if="!wrongValueQuestions && status === 'recommended'" @click="changedStatusForm(user_id, 'approved')" icon="mdi-check-all" size="x-small">
