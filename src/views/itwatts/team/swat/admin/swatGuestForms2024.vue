...
import { exportUser, exportGuestSwat2024Form } from '@/utils/export';

const { t, locale } = useI18n({ useScope: 'global' });
const page = ref({ title: t('swatGuestForms2024.pageTitle') });
...
async function refresh() {
  ...
  try {
    ...
    usersExport.value = `${exportGuestSwat2024Form().headers.concat(exportUser().headers).toString()}\n`;
    for (const form of formsResponse.data.data) {
      ...
      usersExport.value += `${exportGuestSwat2024Form(formContent).value.concat(exportUser(swatUsersMap.get(form.user_id)).value).toString()}\n`;
    }    
  } catch (err: any) {
...
async function exportAll() {
  ...
  const universalBOM = "\uFEFF";
  const file = new File([universalBOM + usersExport.value], `${dateNow} formulaires-invite-2024-5w4t.csv`, {
    type: 'text/csv',
  })
