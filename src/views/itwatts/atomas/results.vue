...
import AtomasResultsComponent from "@/components/itwatts/atomas/AtomasResults.vue";
...
const round = 4;
const page = ref({ title: t('atomasResults.pageTitle', { roundNumber: round})});
const breadcrumbs = ref([
  {
    text: t('atomasResults.pageCategory'),
    disabled: false,
    to: "#",
  },
  {
    text: t('atomasResults.pageTitle', { roundNumber: round}),
    disabled: true,
  },
]);
...
async function refresh() {
  page.value.title = t('atomasResults.pageTitle', { roundNumber: round});
  
  try {
    loading.value = true;
  
    const response = await axios.get(`${config.serverApi.publicHostname}/v1/zp-race-serie-report/atomas_a2024_round2?&round=${round}`, {
