<script setup lang="ts">
  ...
  const overallHeaders: Header[] = [
      ...
      { text: 'cat', value: "cat", sortable: true },
      { text: '95% cp20 w/kg', value: "cp20wkg_95", sortable: true },
      { text: 'cp20 watts', value: "cp20watts", sortable: true },
      /*{ text: 'cp5 w/kg', value: "cp5wkg", sortable: true },
      { text: 'cp1 w/kg', value: "cp1wkg", sortable: true },*/
    ];

  function refresh() {
    categories.value = [];
    if (props.results) {
      if (props.results.signups_per_category) {       
        tabs.value = props.results.signups_per_category[0].category;

        for (let i = 0;i < props.results.signups_per_category.length;i++) {
          categories.value.push(props.results.signups_per_category[i].category);
        }
      }
    }
  }
...
<v-window v-model="tabs">
  <v-window-item v-for="(cat, index) in categories.value" :key="cat" :value="cat">          
    <v-card flat>
      <EasyDataTable v-if="props.results.signups_per_category"
            @click-row="onClickRow"
            :headers="overallHeaders"
            :items="props.results.signups_per_category[index].result"
            alternating
          />
    </v-card>
  </v-window-item>
